...
1. Get your own LLM API key (about 15 minutes)
   ...
   - `uv` lets you configure package mirrors like `pip`. On macOS and Linux, edit `~/.config/uv/uv.toml` and add something like this. On Windows, you can look up the equivalent setup yourself. Run `uv sync` to initialize the project.
   ```
   [[index]]
   ...
   - Configure `OPENAI_API_KEY` and `OPENAI_BASE_URL`, then try running [`core/llm.py`](https://github.com/lasywolf/poipoi-agent/blob/main/core/llm.py).
...
2. Implement Node / Workflow / Agent (about 1 hour)
   ...
   - At the core, an agent can be abstracted with Nodes. I have already prepared a minimal implementation for you in [`core/node.py`](https://github.com/lasywolf/poipoi-agent/blob/main/core/node.py). It takes fewer than 60 lines to build a lightweight agent framework, and it is very easy to understand. If you do not have much Python background, you can paste the code into an AI tool and ask it to explain it.
   - But how do we actually use Nodes? You can create 3 Nodes and connect them to implement `receive input -> search the web -> have the model generate a summary`. Congratulations, you have built a workflow. The relevant implementation is in [`examples/workflow`](https://github.com/lasywolf/poipoi-agent/tree/main/examples/workflow).
   - Now create another workflow for `receive user input -> model responds`, then loop it repeatedly. Congratulations, you have built a chatbot. The implementation is in [`examples/chatbot`](https://github.com/lasywolf/poipoi-agent/tree/main/examples/chatbot).
   - Next, give the chatbot some tools (explained in more detail below) so it can search the web, edit files, and run shell commands. Congratulations, you have built an agent. The implementation is in [`examples/chatbot_with_tools`](https://github.com/lasywolf/poipoi-agent/tree/main/examples/chatbot_with_tools).
...
4. Implement Tools, MCP, and Skills (about 1 hour)
   ...
   - Practice: you can read the implementations in [`tools`](https://github.com/lasywolf/poipoi-agent/tree/main/tools) and [`examples/chatbot_with_tools`](https://github.com/lasywolf/poipoi-agent/tree/main/examples/chatbot_with_tools).
