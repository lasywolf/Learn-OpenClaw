...
5. Implementing Context / Memory (about 10 minutes reading)
   - Short-term Context: The most recent rounds of complete dialogue that haven't been compressed, ensuring the model stays focused on the current task.
   - Long-term Context: Summaries of older dialogue generated by an LLM, kept at the front of the context as a system_message.
   - Short-term Memory = Short-term Context + Long-term Context, stored in the `session.json` file.
   - Long-term Memory: During compression, user preferences, important facts, and similar information are automatically extracted and written into the `MEMORY.md` file.
   - Memory = Short-term Memory + Long-term Memory
   - Why manage conversational memory: Without context management, in order for the model to remember everything done previously, you’d have to feed it all past dialogue (i.e., the full Context). The model would then be overwhelmed by a flood of conversation history (and your tokens burn faster…), its attention would be diluted, and output would slow down.
   - With memory management (context layering + summarization/compression), summarization can drastically shrink the context length (saving tokens), and the tiny token footprint after compression helps the model produce faster, more stable outputs.
   - We’ve built a Memory module to manage all conversational memory. It consists of Short-term Context (the most recent rounds that haven’t been summarized and compressed) and Long-term Context (the already summarized and compressed dialogue content). Summarization into Long-term Context is triggered when the dialogue content reaches 90% of the model’s maximum context length (the actual number of tokens used can be obtained from the `usage` field returned by the LLM API).
   - After compression into long-term memory, the structure fed to the model looks like this:
     ```json
     [
         { // Long-term Context: compressed into a summary as a system prompt
             "role": "system",
             "content": "Conversation History Summary: ...]"
         },
         { // Short-term Context: the most recent complete rounds of dialogue kept
             "role": "user/assistant",
             "content": "recent dialogue content..."
         },
         ...
     ]
     ```
   - During compression, information worth keeping long-term (such as user preferences, important facts, etc.) is also extracted and written to the MEMORY.md file, forming long-term memory that is independent of the ongoing dialogue.
     
   - Practice: See the implementation in [`/core/memory.py`](./core/memory.py) and [`/examples/chatbot_with_memory`](./examples/chatbot_with_memory)folder. After finishing a chat and exiting, you’ll find the session.json and MEMORY.md files in the default memory storage directory.
   
   - Summary: Short-term Context ensures the model focuses on recent context without forgetting what it’s currently doing; Long-term Context lets the model recall earlier events (even if details are fuzzy). The layered design is intended to achieve more KV cache hits — therefore, in the context layering, the front is the most stable, and the back is the least stable.
