...
4. Implement Tools, MCP, and Skills (about 1 hour)
   ...
   - MCP practice: you can read the implementations in [`tools/mcp`](./tools/mcp) and [`examples/chatbot_with_mcp`](./examples/chatbot_with_mcp). The chatbot calls remote `search`/`add`/`multiply` tools on an MCP server through the MCP protocol (stdio).
