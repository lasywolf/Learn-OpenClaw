...
5. Implement Context / Memory Management (approx. 25 minutes to read)
   - Conversation Memory = Conversation Context, written to `session.jsonl`
   - Long-term Memory: user preferences, important facts, and other memorable information automatically extracted during compression, written to ` MEMORY.md `
   - Memory = Conversation Memory + Long-term Memory
   - Why conversation memory management is necessary: as we know, the context window of large models is limited. If the context is not managed, it will eventually exceed the model’s acceptable range, making it impossible to continue. To prevent this, we perform a summarization of the entire conversation just before hitting the maximum context window.
   - After summarization, a lot of information is lost because all previous conversation content is discarded, leaving only the summary. Therefore, we want to minimize the number of summarization passes, which means we always compress the conversation only when we are about to reach the maximum context window.
   - We always keep existing conversation content unchanged, append new messages to `session.jsonl`, and overwrite the summary. The benefit of this approach: large model providers cache conversation information (KV Cache technology). If a longer prefix can be matched in the KV Cache, generation speeds up because the matched prefix can be fetched directly from the cache without regenerating (prefix KV Cache technology).
   - We have implemented a Memory module to manage all conversation information. Trigger for summary generation: when the conversation content reaches or exceeds 90% of the model’s maximum context length (the exact number of tokens used can be obtained from the `usage` field returned by the model API). During compression, information worth remembering long-term (e.g., user preferences, important facts, operating environment) is also extracted and written to ` MEMORY.md `, forming a long-term memory independent of the conversation.
   - Practice: you can read the implementation in [`/core/memory.py`](./core/memory.py) and the [`/examples/chatbot_with_memory`](./examples/chatbot_with_memory) folder. After a conversation, you can find the `session.jsonl` and ` MEMORY.md ` files in the default memory storage folder.
   - Summary: memory management primarily prevents exceeding the context window; summarization ensures the model can recall past events (even if the details are lost). The layered Context design aims to improve KV Cache hit rates, with the most stable part at the beginning and the most volatile part at the end.
